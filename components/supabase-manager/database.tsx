...
function EditRowView({
  ...
  const handleFormSubmit = useCallback(
    (formData: any) => {
      ...
      if (pks.length === 0) {
        toast.error(
          "Cannot update row. This table does not have a primary key."
        )
        return
      }
      ...
      runUpdateQuery(
        { projectRef, query: updateSql, readOnly: false },
        { onSuccess }
      )
    },
    ...
    <div className="p-6 pt-4 lg:p-8 lg:pt-8">
      ...
      {tables && tables.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          {tables.map((table: any) => (
            ...
            >
              ...
              <h2 className="text-sm font-medium font-mono truncate flex-1">
                {table.name}
              </h2>
        ...
        <Alert className="mt-8">
          ...
          <AlertDescription>
            Create tables to store and organize your data.
          </AlertDescription>
        </Alert>
      ) : null}
    </div>
