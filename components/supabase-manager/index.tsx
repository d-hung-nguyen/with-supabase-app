...
import { LogoIncentive } from "@/components/logo-incentive"
              ...
              {navigationItems.map((item) => {
                ...
                return (
                  <Button
                    key={item.title}
                    variant={
                      activeManager === item.title ? "secondary" : "ghost"
                    }
          ...
          <HoverCard>
            <HoverCardTrigger asChild>
              <Link
                ...
                target="_blank"
                className="flex items-center px-4 w-full rounded-none text-sm py-4 h-auto justify-start gap-3 text-left hover:bg-accent"
              >
                <LogoIncentive size={16} />
            ...
            >
              ...
              <p className="text-muted-foreground">
                Access powerful back-end tools for database, auth, storage, and
                logs directly in Supabase.
              </p>
            </HoverCardContent>
          </HoverCard>
        </footer>
    ...
    return (
      <QueryClientProvider client={queryClient}>
        <Dialog open={open} onOpenChange={onOpenChange}>
          <DialogContent className="w-full h-[80vh] max-h-[700px] sm:max-w-[calc(100%-2rem)] w-full] p-0 overflow-hidden sm:rounded-lg">
