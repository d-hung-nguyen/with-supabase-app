...
export function SecretsManager({ projectRef }: { projectRef: string }) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-6 pt-4 lg:p-12 lg:pt-12">
      <div className="mb-16">
        <h2 className="text-base lg:text-xl font-semibold mb-1">
          Add New Secret
        </h2>
        <p className="text-muted-foreground mb-6 text-sm lg:text-base">
          Add sensitive information like API keys that your app needs to work
          securely.
        </p>
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(handleCreateSecrets)}
            className="space-y-2"
          >
              ...
              <div key={field.id} className="flex items-start space-x-2">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      {index === 0 && (
                        <FormLabel className="mb-4">Value</FormLabel>
                      )}
                      <FormControl>
                        <Input
                          type="password"
                ...
                <div className="w-8">
                  {index === 0 && (
                    <FormLabel className="opacity-0 mb-4">Remove</FormLabel>
                  )}
                  <Button
                    type="button"
                    variant="outline"
